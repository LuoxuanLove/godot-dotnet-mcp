...
function Write-DocFixture {
    ...
    Set-Content -LiteralPath (Join-Path $addonRoot "plugin.cfg") -Value "[plugin]`nversion=`"3.0.0`"`n" -Encoding UTF8
    ...
    $paths = @{
        en = @{
            ...
            "overview.md" = "# Overview`n`nEnglish overview content for v3 preparation.`n`n## Release Prep`n`nRead the [v3.0.0 release notes](process/release-notes/release-notes-v3.0.0.md).`n`n## Usage`n`nLine 1.`nLine 2.`nLine 3.`nLine 4.`nLine 5.`nLine 6.`n"
        }
        "zh-CN" = @{
            ...
            "概述.md" = "# 概述`n`n中文概述内容用于 v3 准备。`n`n## 发布准备`n`n阅读 [v3.0.0 发布说明](流程/发布说明/发布说明-v3.0.0.md)。`n`n## 使用`n`n第 1 行。`n第 2 行。`n第 3 行。`n第 4 行。`n第 5 行。`n第 6 行。`n"
        }
        ja = @{
            ...
            "概要.md" = "# 概要`n`n日本語の v3 準備内容です。`n`n## リリース準備`n`n[v3.0.0 リリースノート](プロセス/リリースノート/リリースノート-v3.0.0.md) を確認します。`n`n## 使い方`n`n1 行目。`n2 行目。`n3 行目。`n4 行目。`n5 行目。`n6 行目。`n"
        }
        ko = @{
            ...
            "개요.md" = "# 개요`n`n한국어 v3 준비 내용입니다.`n`n## 릴리스 준비`n`n[v3.0.0 릴리스 노트](프로세스/릴리스-노트/릴리스-노트-v3.0.0.md)를 확인합니다.`n`n## 사용`n`n1번째 줄.`n2번째 줄.`n3번째 줄.`n4번째 줄.`n5번째 줄.`n6번째 줄.`n"
        }
    }
    ...
    $vNextReleaseNotePaths = @{ en = "process/release-notes/release-notes-v3.0.0.md"; "zh-CN" = "流程/发布说明/发布说明-v3.0.0.md"; ja = "プロセス/リリースノート/リリースノート-v3.0.0.md"; ko = "프로세스/릴리스-노트/릴리스-노트-v3.0.0.md" }
    $vNextReleaseNoteBodies = @{
        en = @("## Fixture v3.0.0 Release Notes", "", "Godot .NET MCP v3.0.0 fixture notes cover Companion preparation and validation gates.", "", $vNextReleaseNoteSwitch, "", "### Release Highlights", "", "The fixture describes finished v3 baseline notes for plugin behavior and validation gates.", "", "### Compatibility", "", "Clients should use the v3 preparation guidance for this fixture.")
        "zh-CN" = @("## Fixture v3.0.0 发布说明", "", "Godot .NET MCP v3.0.0 fixture 发布说明覆盖 Companion 准备和验证门禁。", "", $vNextReleaseNoteSwitch, "", "### 发布亮点", "", "该 fixture 描述已经完成的 v3 基线说明、插件行为与验证门禁。", "", "### 兼容性", "", "客户端应使用此 fixture 的 v3 准备指引。")
        ja = @("## Fixture v3.0.0 リリースノート", "", "Godot .NET MCP v3.0.0 fixture notes は Companion 準備と validation gate を扱います。", "", $vNextReleaseNoteSwitch, "", "### Release Highlights", "", "この fixture は完了済みの v3 baseline notes、plugin behavior、validation gate を説明します。", "", "### Compatibility", "", "client はこの fixture の v3 準備ガイダンスを使用します。")
        ko = @("## Fixture v3.0.0 릴리스 노트", "", "Godot .NET MCP v3.0.0 fixture notes는 Companion 준비와 validation gate를 다룹니다.", "", $vNextReleaseNoteSwitch, "", "### Release Highlights", "", "이 fixture는 완료된 v3 baseline notes, plugin behavior, validation gate를 설명합니다.", "", "### Compatibility", "", "client는 이 fixture의 v3 준비 지침을 사용합니다.")
    }
    foreach ($locale in $vNextReleaseNoteBodies.Keys) {
        $paths[$locale][$vNextReleaseNotePaths[$locale]] = ($vNextReleaseNoteBodies[$locale] -join "`n") + "`n"
    }
