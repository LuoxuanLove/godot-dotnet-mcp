...
if ([string]::IsNullOrWhiteSpace($projectVersion)) {
    throw "Roslyn bridge project must declare a Version property."
}
...
try {
    ...
    if ([string]$manifest.version -ne $projectVersion) {
        throw "Roslyn runtime manifest version '$($manifest.version)' must match DotnetBridge project Version '$projectVersion'."
    }
    ...
    $depsPath = Join-Path $runtimeDirectory "GodotDotnetMcp.PluginBridge.deps.json"
    if (Test-Path -LiteralPath $depsPath) {
        $deps = Get-Content -LiteralPath $depsPath -Raw -Encoding UTF8 | ConvertFrom-Json
        $expectedTarget = "GodotDotnetMcp.PluginBridge/$projectVersion"
        $targetIdentities = @($deps.targets.PSObject.Properties | ForEach-Object {
            $_.Value.PSObject.Properties.Name
        })
        if ($targetIdentities -notcontains $expectedTarget) {
            $errors.Add("Roslyn runtime deps targets must include '$expectedTarget'. Actual targets: $($targetIdentities -join ', ')")
        }
    }

    $runtimeExePath = Join-Path $runtimeDirectory "GodotDotnetMcp.PluginBridge.exe"
    if (Test-Path -LiteralPath $runtimeExePath) {
        $capabilitiesJson = & $runtimeExePath --capabilities
        if ($LASTEXITCODE -ne 0) {
            $errors.Add("Bundled Roslyn runtime --capabilities failed with exit code $LASTEXITCODE.")
        } else {
            try {
                $capabilities = $capabilitiesJson | ConvertFrom-Json
                if ([string]$capabilities.version -ne $projectVersion) {
                    $errors.Add("Bundled Roslyn runtime capabilities version '$($capabilities.version)' must match DotnetBridge project Version '$projectVersion'.")
                }
            } catch {
                $errors.Add("Bundled Roslyn runtime --capabilities did not return valid JSON: $($_.Exception.Message)")
            }
        }
    }
