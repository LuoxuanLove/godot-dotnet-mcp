...
$bannedReadmeInstallPatterns = @(
    ...
    @{ Pattern = '(?i)\bcopy source files directly\b|\bdirect copy of the `addons/godot_dotnet_mcp/` source files\b|\bcopy (?:the )?raw repository\b|\bcopy raw source\b|^###\s+Copy from source\s*$|^###\s+从源码复制\s*$|^###\s+ソースから\s*$|^###\s+소스에서 설치\s*$'; Description = "raw source-copy install wording" }
)
foreach ($relativeReadmePath in $releaseFacingReadmes) {
    ...
    foreach ($requiredInstallText in @("plugin/runtime/roslyn_runtime/", "dotnet_bridge/")) {
        if (-not $readmeText.Contains($requiredInstallText)) {
            $errors.Add("Release-facing README must document prepared installable addon boundaries and mention '$requiredInstallText': ${relativeReadmePath}.")
        }
    }
}
