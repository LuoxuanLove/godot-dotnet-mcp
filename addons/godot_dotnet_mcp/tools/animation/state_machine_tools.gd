...
## Animation state machine tools for Godot MCP
...
func _list_transitions(sm: AnimationNodeStateMachine) -> Dictionary:
	...
	for i in range(sm.get_transition_count()):
		...
		transitions.append({"from": from_node, "to": to_node, "advance_mode": transition.advance_mode, "switch_mode": transition.switch_mode, "xfade_time": transition.xfade_time})
