...
func execute_async(tool_name: String, args: Dictionary) -> Dictionary:
	match tool_name:
		"inspect":
			return await _execute_inspect_async(args)
		"symbols":
			return await _execute_symbols_async(args)
		"exports":
			return await _execute_exports_async(args)
		_:
...
func _execute_inspect_async(args: Dictionary) -> Dictionary:
	var path = args.get("path", "")
	if path.is_empty():
		return _error("Path is required")

	var parse_result = await _parse_csharp_metadata_via_roslyn_async(path) if _normalize_res_path(path).ends_with(".cs") else _parse_script_metadata(path)
	if not parse_result.get("success", false):
		return parse_result
...
func _execute_symbols_async(args: Dictionary) -> Dictionary:
	var path = args.get("path", "")
	if path.is_empty():
		return _error("Path is required")
	...
	return _build_symbols_payload(path, args, parse_result)
...
func _execute_exports_async(args: Dictionary) -> Dictionary:
	var path = args.get("path", "")
	if path.is_empty():
		return _error("Path is required")
	...
	return _build_exports_payload(path, parse_result)
...
func _parse_csharp_metadata_via_roslyn_async(path: String) -> Dictionary:
	var normalized_path := _normalize_res_path(path)
	var service = _get_plugin_roslyn_service()
	if service == null:
		return _error("Plugin Roslyn service is unavailable")
	var result
	if service.has_method("parse_file_async"):
		result = await service.parse_file_async(normalized_path)
	else:
		result = service.parse_file(normalized_path)
	if not bool(result.get("success", false)):
		return result
	return result


func _build_symbols_payload(path: String, args: Dictionary, parse_result: Dictionary) -> Dictionary:
	var kind_filter = str(args.get("kind", "")).strip_edges()
	var query = str(args.get("query", "")).to_lower()
	var symbols: Array = []
...
func _build_exports_payload(path: String, parse_result: Dictionary) -> Dictionary:
	var metadata = parse_result["data"]
	var payload := {
		"path": str(metadata.get("path", _normalize_res_path(path))),
		"language": str(metadata.get("language", "unknown")),
