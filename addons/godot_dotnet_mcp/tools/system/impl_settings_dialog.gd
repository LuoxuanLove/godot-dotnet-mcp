	...
	return [
		{
			...
			"inputSchema": {
				...
				"properties": {
					...
					"setting_path": {
						"type": "string",
						"description": "Optional setting path or path fragment. search writes this text into the settings filter; list_rows/read_value/verify_value only filter currently observed visible rows."
					},
					"tab": {
						"type": "string",
					...
					"target_path": {
						"type": "string",
						"description": "Control path returned by status/search/list_rows for focus_result, or a row/value control path used by read_value/verify_value"
					},
					"expected_value": {
					...
					"require_confidence": {
						...
						"description": "Minimum row model confidence accepted by read_value/verify_value (default medium)"
					},
					"include_hidden": {
						"type": "boolean",
...
func _verify_number_value(actual_value, expected_value) -> Dictionary:
	var actual_number := _coerce_verify_number(actual_value)
	var expected_number := _coerce_verify_number(expected_value)
...
func _coerce_verify_number(value) -> Dictionary:
