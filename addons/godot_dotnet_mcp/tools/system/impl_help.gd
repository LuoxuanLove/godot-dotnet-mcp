...
func _build_help(include_tools: bool) -> Dictionary:
	...
	var payload := {
		...
		"recommended_start": [
			...
			"Use system_settings_dialog for Project Settings or Editor Settings open/status/search/list_tabs/activate_tab/list_categories/focus_category/list_rows/resolve_row/read_value/focus_value/set_value/verify_value/focus_result/capture/close workflows before falling back to raw editor control enumeration; capture defaults to the settings popup/window when one is visible and reports capture_surface metadata.",
		...
		"visual_guidance": {
			...
			"settings_dialog_capture": "system_settings_dialog(action=capture, capture_mode=auto|editor|popup)",
