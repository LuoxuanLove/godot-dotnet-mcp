...
func _build_help(include_tools: bool) -> Dictionary:
	...
	var payload := {
		...
		"recommended_start": [
			...
			"Use system_settings_dialog for Project Settings or Editor Settings open/status/search/list_rows/read_value/set_value/verify_value/focus/capture/close workflows before falling back to raw editor control enumeration.",
