...
func get_tools() -> Array[Dictionary]:
	return [
		{
			"name": "tool_catalog",
			"description": "TOOL CATALOG: Search the current tool catalog by query, category, or domain. ACTIONS: search (default). Returns matching tools with exposed/enabled state, actions, parameters, group path, and match reasons. Use visibility=visible to include non-public visible internal tools.",
			"inputSchema": {
				"type": "object",
				"properties": {
					"action": {"type": "string", "enum": ["search"], "description": "Tool catalog action (default: search)"},
					"query": {"type": "string", "description": "Optional case-insensitive search text matched against tool names, descriptions, actions, parameters, enum values, category, and domain."},
					"category": {"type": "string", "description": "Optional tool category filter, such as system, project, editor, runtime, script, or user."},
					"domain": {"type": "string", "description": "Optional domain key filter, such as core, project, editor, runtime, analysis, plugin, or user."},
