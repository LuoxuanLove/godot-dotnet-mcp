...
func _build_editor_ui_control_prompt(arguments: Dictionary) -> Dictionary:
	...
	var text := _text("prompt_editor_ui_control_body", "Use when: Start here when a task depends on the current Godot editor UI, including docks, plugin panels, bottom panels, popups, settings dialogs, layout, focus, button visibility, screenshots, or control-local text and clicks. Recommended workflow: 1. Call system_editor_state to capture the current main screen, selections, inspector summary, runtime state, and editor capability context. 2. For Project Settings or Editor Settings, use system_settings_dialog to open, observe, search, list and focus category tree items, list read-only row models, read current visible row values, focus a returned result, capture, or close the settings surface before falling back to raw controls. 3. Use system_editor_control(action=activate_ui), activate_dock_tab, or set_main_screen to bring other editor surfaces into view without OS window automation. 4. Capture evidence with system_editor_control(action=capture_editor) before judging layout or visibility. 5. Call list_controls with include_hidden=false first, then retry include_hidden=true when the visible enumeration misses the target; use get_control or capture_control before interacting. 6. Dispatch focus_control, activate_control, click_control, right_click_control, set_control_text, list_popups, press_popup_button, or close_popup using Control-local coordinates and popup paths returned by the tools. Validation: capture or list the resulting control state, popup state, settings dialog status, or editor log evidence after the UI action. Avoid: do not use OS mouse or window automation unless explicitly authorized, do not guess screen coordinates, and do not use editor UI control when scene_patch or script_patch can safely make the file-level change.")
