...
internal sealed class StdioMcpServer
{
    ...
    public StdioMcpServer(Stream output, TextWriter error)
    {
        _output = output;
        _error = error;
    }
    ...
    {
        await _error.WriteLineAsync($"Notification received: {request.Method}");
        await _error.FlushAsync();
    }
    ...
    private async Task HandleRequestAsync(JsonRpcRequest request, CancellationToken cancellationToken)
    {
        object? result = request.Method switch
        {
            ...
            "ping" => new { },
            "tools/list" => new { tools = BridgeToolCatalog.GetTools() },
    ...
    private async Task<object> HandleToolCallAsync(JsonRpcRequest request, CancellationToken cancellationToken)
    {
        ...
        var toolResult = await BridgeToolDispatcher.ExecuteAsync(toolName, toolArguments, cancellationToken);
        return new
        {
            content = new[]
            {
    ...
    private async Task WriteResultAsync(string? id, object? result, CancellationToken cancellationToken)
    {
        ...
        await BridgeApplication.WriteJsonAsync(_output, payload, cancellationToken);
    }
