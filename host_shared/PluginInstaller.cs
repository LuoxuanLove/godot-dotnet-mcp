...
internal static class PluginInstaller
{
    ...
    private const string CustomToolsDirectoryName = "custom_tools";
    ...
    public static async Task<int> RunAsync(string[] args, Stream output, TextWriter error, CancellationToken cancellationToken)
    {
        ...
        {
            await error.WriteLineAsync("Plugin source directory not found. Pass --source-path or package the plugin folder next to the executable.");
            return 2;
        }

        var targetRoot = Path.Combine(projectRoot, PluginRelativePath);
        string? preservedCustomToolsPath = null;
        var preservedCustomTools = false;
        ...
        try
        {
            if (Directory.Exists(targetRoot))
            {
                ...
                preservedCustomToolsPath = PreserveCustomToolsIfPresent(projectRoot, targetRoot);
                preservedCustomTools = !string.IsNullOrWhiteSpace(preservedCustomToolsPath);
                Directory.Delete(targetRoot, recursive: true);
            }

            Directory.CreateDirectory(targetRoot);
            var fileCount = CopyDirectory(sourceRoot, targetRoot, overwrite: true);
            var restoredCustomTools = RestorePreservedCustomTools(targetRoot, preservedCustomToolsPath);
            ...
            var payload = new
            {
                ...
                preservedCustomTools = restoredCustomTools,
                message = preservedCustomTools
                    ? "Plugin installed successfully and preserved custom_tools."
                    : "Plugin installed successfully."
            };

            await BridgeSerialization.WriteJsonAsync(output, payload, cancellationToken);
            return 0;
        }
        catch (Exception ex)
        {
            await error.WriteLineAsync($"Plugin install failed: {ex.Message}");
            if (!string.IsNullOrWhiteSpace(preservedCustomToolsPath) && Directory.Exists(preservedCustomToolsPath))
            {
                await error.WriteLineAsync($"Preserved custom_tools can be recovered from: {preservedCustomToolsPath}");
            }
    ...
    private static string? PreserveCustomToolsIfPresent(string projectRoot, string targetRoot)
    {
        var customToolsPath = Path.Combine(targetRoot, CustomToolsDirectoryName);
        if (!Directory.Exists(customToolsPath))
        {
            return null;
        }

        var preservePath = Path.Combine(projectRoot, $".godot_dotnet_mcp_preserve_{Guid.NewGuid():N}");
        Directory.Move(customToolsPath, preservePath);
        return preservePath;
    }

    private static bool RestorePreservedCustomTools(string targetRoot, string? preservedCustomToolsPath)
    {
        if (string.IsNullOrWhiteSpace(preservedCustomToolsPath) || !Directory.Exists(preservedCustomToolsPath))
        {
            return false;
        ...
        var restorePath = Path.Combine(targetRoot, CustomToolsDirectoryName);
        Directory.Move(preservedCustomToolsPath, restorePath);
        return true;
    }
