...
func run_case(_tree: SceneTree) -> Dictionary:
	...
	if get_mcp_body.find("event: endpoint") == -1 or get_mcp_body.find("\"transport\":\"streamable_http\"") == -1:
		return _failure("HTTP request router should return an observable SSE endpoint event for GET /mcp.")
