...
> 一个面向 Godot 4.6+ 的编辑器 MCP 插件，通过 MCP 暴露当前 Godot 编辑器中的项目状态、场景编辑、脚本分析、运行时控制、截图、日志和客户端接入配置。

Godot .NET MCP 面向需要当前编辑器状态的 MCP 客户端，而不是只读取磁盘文件的工具。它提供读取项目健康状态、检查场景、修改脚本、控制运行、截图、读取日志和配置客户端的 MCP 工具；C# 分析由插件内 Roslyn 语法层完成，不需要外部宿主进程。

![Godot .NET MCP 工具页](asset_library/tools-cn.png)

## 为什么需要它

Godot 编辑器工作常依赖文件之外的状态：当前打开的场景、Dock 状态、运行时报错、编辑器界面、客户端配置、截图结果，以及工具是否已经在当前会话中可用。Godot .NET MCP 将这些状态和操作整理为明确的 MCP 工具，让客户端在修改场景、脚本、资源或运行时状态前先读取编辑器实际状态。
...
- **可视化编辑器感知**：Agent 可以截取编辑器界面、枚举可见或隐藏控件，并通过 Godot 编辑器 API 激活 Dock、底部面板、控件和弹窗。
...
`v1.0.0-pre2` 是当前预发布版本，覆盖 System 工具层、编辑器 / 运行时自动化、客户端接入配置、插件内 Roslyn 支持、用户工具热重载、本地化 Dock UI 与发布验证。完整版本记录见 [CHANGELOG.zh-CN.md](CHANGELOG.zh-CN.md)。
