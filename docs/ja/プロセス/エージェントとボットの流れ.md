...
- 通常の integration PR は `dev` 向けです。明示的な v3 predevelopment PR は `v3.0`、`release/v3.0.0-baseline`、または anchored `feature/v3-*`、`docs/v3-*`、`ci/v3-*` stacked base branch を target にできます。それ以外の target branch は `pr-policy` が fail にします
...
1. PR の target は許可された integration branch または明示的な v3 stacked base で、今の検証可能な target base に対して再確認している
