...
2. `pr-policy`: 間違った target branch の PR を止める。stable integration branch と明示的な v3 stacked base だけを許可し、title、summary、testing などの客観字段を確認する
3. `version-policy`: 信頼された policy workflow と script で PR の公開 version metadata を検証し、実際の PR base/head ref を比較して、`release/*` 以外の branch が早く version を変えるのを防ぐ
...
8. `publish-plugin`: pushed `v*` tag または manual release check を検証し、tag version、`dev` 到達性、release note source file、build、harness、rendered release-note artifact を確認する。正式 GitHub Release は作成しない
...
- PR target branch と軽い PR standards check。間違った target branch なら失敗して、許可された integration branch または明示的な v3 stacked base へ投げ直すよう伝える。必要字段が欠けていれば補うよう伝える
- dotnet bridge library、harness runner、fixture build
- refactor guardrails
- plugin headless harness required subset
- release validation 前の `plugin.cfg`、tag、4 言語すべての changelog、4 言語すべての `v*` release-note source file の version 一致確認
