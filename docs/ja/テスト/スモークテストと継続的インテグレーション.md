...
2. `pr-policy`: 間違った target branch の PR を止める。stable integration branch と明示的な v2 stacked base だけを許可し、title、summary、testing などの客観字段を確認する
...
- PR target branch と軽い PR standards check。間違った target branch なら失敗して、許可された integration branch または明示的な v2 stacked base へ投げ直すよう伝える。必要字段が欠けていれば補うよう伝える
