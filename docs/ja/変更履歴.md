...
- `system_plugin_maintenance` を追加し、status、reload、update status、source selection、update start workflow を 1 つの plugin maintenance entry にまとめました。`system_plugin_reload` と `system_plugin_update` は引き続き利用できます。
...
- tool schema facts を `2026-06-05.7` に更新し、`system_plugin_maintenance_contracts` で grouped maintenance entry と Tools tree localization を検証するようにしました。
