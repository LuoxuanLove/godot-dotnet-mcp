...
- `system_settings_dialog(action=list_categories/focus_category)` を追加しました。row inspection の前に settings category tree を発見し、一意の category に focus できます。setting value は書き込みません。
...
- settings dialog category navigation のため、editor UI control atomic layer に Tree item listing / selection coverage を追加しました。
