...
- 最小限の `GET /mcp` Streamable HTTP SSE probe surface を追加しました。`Accept: text/event-stream` を必須にし、protocol-version header を検証し、session/protocol response header を返し、client transport discovery 用の小さな endpoint event を送信します。
