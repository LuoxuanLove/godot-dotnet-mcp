...
- `system_settings_dialog(action=capture, capture_mode=auto|editor|popup)` を追加しました。settings evidence は active popup / window を優先、full editor capture を強制、または popup-only capture を要求でき、`capture_surface` metadata と明示的な fallback detail を返します。
