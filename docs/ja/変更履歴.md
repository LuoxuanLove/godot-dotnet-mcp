...
- editor startup と idle refresh を軽量化し、auto-start を `_enter_tree` の同期 path から外し、非 active Dock tab を lazy-load し、MCP server startup を軽く保ち、tool runtime を on demand で読み込み、現在の Dock tab に必要な catalog projection だけを構築し、User-tool file polling の頻度を下げることで large projects での editor stalls を減らしました。
