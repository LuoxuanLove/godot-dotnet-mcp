...
- `system_project_configure(action="list_export_presets")` を追加し、platform option value を返さずに `export_presets.cfg` を read-only summary として確認できるようにしました。
...
- export preset summary discovery を検証する system project executor と Tools-tab rendering coverage を追加しました。
