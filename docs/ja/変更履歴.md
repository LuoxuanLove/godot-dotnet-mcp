...
- `system_tool_catalog` を追加し、client が現在公開されている tool catalog を query、category、domain で検索し、match reason、action、parameter、visibility、enabled state を確認できるようにしました。
- `system_project_configure(action="list_export_presets")` を追加し、sensitive option key と local absolute export path を redact しながら `export_presets.cfg` を read-only summary として確認できるようにしました。
- `system_project_configure(action="get_input_action")` を追加し、Agent が高レベルの project configuration tool から個別の input action の deadzone と具体的な event binding を確認できるようにしました。
- `system_editor_control(action="wait_for_ui")` を追加しました。editor control の存在、可視性、text、enabled / disabled 条件を有界にポーリングし、成功時は一致した UI evidence、失敗時は timeout payload を返します。
- `system_editor_control(action="get_popup")` と `system_editor_control(action="capture_popup")` を追加しました。返された popup path から可視の floating editor popup / window を読み取り、局所的な evidence を capture できます。
- `system_settings_dialog` を追加しました。Project Settings / Editor Settings を開き、可視化を待ち、候補 setting row を検索し、結果に focus し、evidence を capture して、setting value を書き込まずに surface を閉じる high-level settings-like dialog workflow です。
- `system_settings_dialog(action=list_tabs/activate_tab)` の settings tab navigation を追加しました。`open(tab=...)` も含め、座標クリックなしで Project Settings / Editor Settings の tab を list / switch できます。
- `system_settings_dialog(action=list_categories/focus_category)` を追加しました。row inspection の前に settings category tree item を発見し、一意の category に focus できます。setting value は書き込みません。
- 読み取り専用の `system_settings_dialog(action=list_rows)` row model を追加しました。settings search field を書き込まず、可視 settings-like control を保守的な confidence と evidence 付きで要約します。
- 読み取り専用の `system_settings_dialog(action=read_value)` value inspection を追加しました。一意に一致した可視 setting row について、row/value evidence とともに text、bool、number、enum の型付き payload を返し、setting value は書き込みません。
- 読み取り専用の `system_settings_dialog(action=resolve_row)` 行解決を追加しました。値の読み取りや後続 UI 操作の前に、一意の可視設定行を確認できます。
- 読み取り専用の `system_settings_dialog(action=focus_value)` value-editor focus を追加しました。一意に一致した可視 setting row の value control に focus を移し、setting value は書き込みません。
- `system_settings_dialog(action=set_value)` を追加しました。対応している一意の可視 setting row について、editor UI control 経由で text、number、bool 値を書き込み、操作後に観測値を再確認します。
- 読み取り専用の `system_settings_dialog(action=verify_value)` value assertion を追加しました。一意に一致した可視 setting row について、期待する text、bool、number、enum 値を比較し、setting value は書き込みません。
- `system_settings_dialog(action=run_task)` を追加しました。open / navigation、unique row resolution、before read、optional supported write、verification、capture evidence を 1 つの high-level trusted settings task workflow にまとめます。
- `system_inspector` を追加しました。現在編集中の object または準備済み node/resource target について、可視 property model の list、一意な property の resolve、read、focus、set、verify、capture、trusted property task を行う high-level Inspector property workflow です。
- `system_project_lifecycle(action=start|stop)` を canonical な high-level project runtime session lifecycle entry として追加し、古い project run / stop tool 名の compatibility alias は保持しないようにしました。
- `system_editor_evidence` を追加しました。editor、control、popup、active-dialog、auto surface を capture し、target、fallback、degradation、visible-popup metadata を明示する high-level visual evidence workflow です。
- `system_scene_inspect` を追加しました。`validate`、`analyze`、`full` action を備えた統一 read-only scene inspection entry で、既存の scene validation / analysis tool も引き続き利用できます。
- `system_plugin_maintenance` を追加しました。status、reload、update status、update source selection、update start workflow をまとめる high-level plugin maintenance entry で、専用 reload / update tool は引き続き利用できます。
- User tool naming diagnostics を追加しました。User tool listing と compatibility report で、script が宣言した名前、正規化後の名前、公開 MCP tool name を返します。
- `system_tool_activity` query filter と slow / failure diagnostics を追加しました。client は state、tool、slow-call threshold で running / recent call を確認でき、無関係な activity record を取得せずに済みます。
- `system_editor_control` の click fallback observation evidence を追加しました。control-local の left/right click について、dispatch された input event、Button-like target の click 前後 state、観測された control signal、input が届いても activation evidence が観測されない場合の diagnostic hint を返します。

### Changed

- editor UI control guidance を明確化し、Agent が semantic workflow / navigation を最優先し、control-level の focus/text/value/popup action を次に使い、control-local mouse / pointer event は fallback としてのみ使うようにしました。
...
- MCP resources/prompts harness coverage を拡張し、non-English locale で resource と resource template metadata の localization を検証するようにしました。
- Config client registry、production detection path、presenter の contract coverage を拡張し、capability matrix projection、compatibility alias、fallback behavior を検証するようにしました。
- export preset summary discovery、sensitive option-key redaction、absolute export-path redaction を検証する system project executor と Tools-tab rendering coverage を追加しました。
- tool catalog search の harness coverage を追加し、public exposure、internal visible lookup、schema opt-in、filter handling、match-reason reporting を検証するようにしました。
- plugin metadata、protocol facts、.NET bridge metadata、plugin-update contract fixture expectation、localized changelog、release-note source を `1.2.1` development line へ切り替えました。
- settings-dialog category navigation のため、Tree item listing / selection を editor UI control atomic layer に追加しました。
- Tools ページの rendering contract と文書例を拡張し、`plugin` domain も core / user tool domain と並んで coverage に含めました。
- Tools ページの header count を修正し、`plugin` domain の tool も enabled / total tool 数に含めるようにしました。
- canonical な `system_project_lifecycle` surface に合わせて、project lifecycle tool tree、prompt、localization、protocol facts、stdio/router/tool-loader、harness contract を更新し、`2026-06-08.16` tool schema facts update と明示的な lifecycle action validation を追加しました。
- Settings page rendering coverage を拡張し、長い discovered update ref list がすべての selectable option を保持しつつ selector popup height を制限することを検証しました。
- semantic-first、control-level-second、mouse-fallback automation order のため、editor UI prompt、help、localization、schema contract coverage を更新し、`2026-06-08.17` tool schema facts update を追加しました。
- settings-dialog task contract coverage を拡張し、`run_task` の read / verify / set / set-and-verify flow、ambiguity と write-refusal guard、capture-required write preflight、Tools page action rendering、catalog search discovery、prompt/help guidance、`2026-06-08.19` tool schema facts update を検証するようにしました。
- Inspector workflow contract と tool-tree coverage を追加し、property model resolution、typed value read、guarded text/number/bool write、capture fallback、catalog discovery、prompt/help guidance、`2026-06-08.20` tool schema facts update を検証するようにしました。
- editor evidence surface contract、tool-tree entry、Tools tab rendering、catalog search discovery、prompt/help guidance、localized tool metadata、`2026-06-08.21` tool schema facts update を追加しました。
- scene executor、tool loader、Tools tab rendering、localization inventory、protocol facts coverage を拡張し、`2026-06-08.23` の `system_scene_inspect` schema surface を検証するようにしました。
- grouped maintenance entry 向けに plugin-maintenance contract、tool-tree coverage、localized tool metadata、`2026-06-08.22` tool schema facts update を追加しました。
- User tool service contract coverage を拡張し、scaffold public-name preview、旧式 `user_` prefix normalization warning、`2026-06-08.24` tool schema facts update を検証しました。
- tool activity registry と loader contract を拡張し、filtered status / recent query、live running duration、slow-call summary、bounded recent failure summary、`2026-06-08.25` tool schema facts update を検証しました。
- editor/system control harness coverage を拡張し、semantic-first UI automation guidance を維持したまま click input dispatch と Button-like target observation evidence を検証しました。

### Fixed

- MCP resource diagnostics の redaction を強化し、mixed-case URL credential と sensitive text marker が resource output として報告される前にマスクされるようにしました。
...
- 生成される User tool scaffold が base tool script を resource path で継承するように修正しました。headless catalog scan が global script-class registration に依存しません。
