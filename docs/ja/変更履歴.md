...
- `system_project_execution(action=run|stop)` を統一 high-level project lifecycle entry として追加しました。既存の project-run marker validation、foreground policy、timeout、auto-stop、stop behavior を再利用し、`system_project_run` と `system_project_stop` の互換性は維持します。
- MCP resources と prompts に output size safeguard を追加しました。過大な file-backed resource は全体読み込み前に拒否し、過大な prompt text は `_meta` の byte-size detail 付きで切り詰めます。
...
- `system_tool_activity` と MCP ツール呼び出し用の任意 `_mcp_context` メタデータを追加しました。個別ツールの schema を変えずに、実行中の呼び出し、最近の完了履歴、実行順序、Agent の自己申告による協調コンテキストを軽量に確認できます。
...
- `plugin_evolution_runtime_diagnostics` と `project_state(include_runtime_health=true)` から User tool runtime diagnostics を追加し、検出数、読み込み失敗、watcher 状態、互換性 summary、最近の監査記録を確認できるようにしました。
- `system_editor_control(action=select_popup_menu_item)` を追加し、agent が visible な `PopupMenu` item を index、id、完全一致 text で選択できるようにしました。hidden popup、disabled item、separator、submenu row、競合する selector は拒否します。
...
- `system_editor_control` に hover と leave の pointer-motion action を追加し、OS mouse automation を使わずに tooltip、hover-only menu、floating editor UI を検証できるようにしました。
- `system_editor_plugin_control` を追加し、client が third-party EditorPlugin の project-setting state、editor-session state、visible UI hint、restart / manual-activation guidance、保護付き enable / disable behavior を確認できるようにしました。
- `system_editor_control` に high-level editor control action を追加し、現在の main-screen button の列挙、登録済み plugin main screen への切り替え、distraction-free mode の読み取りと切り替えを行えるようにしました。

### Fixed

- malformed JSON-RPC parameter handling を修正し、HTTP と stdio request が object ではない top-level `params` を一貫して `-32602` で拒否するようにしました。一方で object ではない `tools/call.arguments` は tool-level の `isError` result を維持します。
...
- C# の空 method 生成を修正し、body がない場合に内部 edit helper が曖昧な fallback body を出さないようにしました。
...
- project file reimport の処理を修正し、`project.godot`、text file、sidecar metadata、未対応 path は Godot import pipeline に渡さず、構造化された `not_importable_resource` error を返すようにしました。

### Internal

- system project executor、tool loader、Tools page、runtime-state、localization、tool-tree harness coverage を拡張し、`system_project_execution` を検証しました。tool schema facts は `2026-06-06.2` に更新しました。
- JSON-RPC request、HTTP server、stdio resources / prompts、tool router contract を拡張し、malformed `params` と `tools/call.arguments` boundary を `2026-06-05.6` tool schema facts update とともに検証するようにしました。
- `mcp_resources_prompts_contracts` を拡張し、resource output limit、prompt truncation metadata、`2026-06-05.5` tool schema facts update を検証するようにしました。
- script edit service harness coverage を拡張し、C# method body がない場合は曖昧な fallback body ではなく明示的な `NotImplementedException` guard body を要求するようにしました。
- User tool service、runtime health、project-state harness coverage を拡張し、runtime diagnostics と custom-tool load failure reporting を検証するようにしました。
- plugin-evolution harness coverage を拡張し、公開 runtime diagnostics が live user-tool runtime snapshot を転送し続けることを検証するようにしました。
- tool schema facts version を更新し、`list_menus`、`open_menu`、`select_menu_item` の editor UI harness coverage を追加しました。
- ツール活動 registry の router、stdio、registry、tool-loader harness 契約を追加し、context の剥離、レスポンス activity 概要、transport coverage、公開 `system_tool_activity` エントリを検証するようにしました。
- Tools page 文書を更新し、`system_tool_activity` tree entry と `_mcp_context` protocol boundary を追加しました。
...
- DAP debugger に oversized `Content-Length` frame と `timeout_ms` の上限保護を追加し、異常な debug endpoint が長い timeout を待たずに `dap_limit_exceeded` を返せるようにしました。
- plugin harness の failure report に `failureClasses`、`primaryFailureClass`、`exitCleanupWarningFailure` を追加し、Godot exit cleanup warning が runtime error marker を隠さないようにしました。
- editor-control harness と tool tree coverage を拡張し、動的な main screen discovery、plugin main-screen activation、distraction-free action を検証するようにしました。
