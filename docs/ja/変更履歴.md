...
- `system_scene_inspect` を統一された read-only scene inspection entry として追加しました。`validate`、`analyze`、`full` action を提供し、既存の scene validation / analysis tool との互換性を維持します。
...
- scene executor、tool loader、Tools tree rendering、locale inventory、protocol facts の coverage を拡張し、`2026-06-06.1` の `system_scene_inspect` schema surface を検証するようにしました。
