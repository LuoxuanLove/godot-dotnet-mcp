...
- Config client card 向けに安定した capability matrix を追加し、support level、supported action metadata、note key を出せるようにしました。
...
- Config client detector と presenter の契約 coverage を拡張し、capability matrix projection、互換 alias、fallback behavior を検証するようにしました。
