...
- `system_editor_control` と low-level `ui_control` の `activate_control` に activation observation evidence を追加しました。Button-like target の activation 前後 state、observed signal、semantic activation が完了しても observable evidence がない場合の hint を返します。
...
- editor/system control harness coverage を拡張し、`activate_control` target observation metadata と activation evidence を検証しました。
