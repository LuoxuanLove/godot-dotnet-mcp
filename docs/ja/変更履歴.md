...
- stdio frame processing に async reentry guard を追加し、tool-loader tick を維持したまま連続 stdio request の安定性を高めました。
...
- HTTP transport、health、plugin reload、plugin update の harness contract を拡張し、pipelined request と再接続対応 client 向けの maintenance window response field を検証するようにしました。
