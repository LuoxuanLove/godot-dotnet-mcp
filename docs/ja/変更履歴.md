...
- legacy internal compatibility alias の `filesystem_file` を削除しました。file operation には `filesystem_file_read`、`filesystem_file_write`、`filesystem_file_manage` を使います。
...
- `filesystem_file` が loader definition、filesystem domain の直接実行、localization inventory に戻らないよう removal guard coverage を追加し、split filesystem tools は引き続き利用可能にしました。
