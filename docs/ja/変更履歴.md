...
- v3.0 初の .NET Companion contract library と pure contract runner を追加し、project sessions、capability-gated static/headless mode、明示的な editor-live upgrade、project/session-scoped tool calls をカバーしました。
...
- v3 capability manifest と validator を追加し、static/headless capability が editor-live state、default background process、listening port、または scope のない tool call を宣言できないようにしました。
- 明示的な Broker project removal contract を追加し、registered project を削除してその active project sessions を revoke しつつ、他の project には影響しないようにしました。
- 非変更型の editor-live upgrade eligibility contract を追加し、client が project session を明示的に editor-live mode へ切り替える前に upgrade blocker を報告できるようにしました。
- project-scoped Broker session listing contract を追加し、client が単一の `project_id` の active sessions を project boundary を越えず lease も更新せずに検査できるようにしました。
- Broker editor bridge status snapshot contract を追加し、project-scoped bridge state を記録しても project sessions が暗黙に upgrade されないようにしました。
- Broker-owned editor-live upgrade contract を追加し、project/session-scoped tool calls が同じ project に保存された bridge status からのみ session を切り替えられるようにしました。
- Broker-owned editor-live upgrade preflight contract を追加し、client が保存済みの bridge blockers を session の変更や lease 更新なしに読めるようにしました。
- Broker session capability snapshot contract を追加し、client が session lease を更新せずに scoped static/headless または editor-live capabilities を検査できるようにしました。
- Broker tool availability preflight contract を追加し、client が scoped tools を呼び出す前に required capabilities と blockers を検査できるようにしました。
- Broker session health snapshot contract を追加し、client が session lease を更新せずに scoped static/headless または editor-live state、bridge status、editor-live upgrade eligibility、current capabilities を読めるようにしました。
- Broker status snapshot contract を追加し、client が filesystem scan、Godot launch、session lease renewal なしに registered project counts、active session mode counts、bridge status counts、project summaries を検査できるようにしました。
- Broker project health snapshot contract を追加し、client が scan、Godot launch、lease renewal なしに単一の `project_id` の project file scope、active session mode counts、bridge status、editor-live upgrade eligibility を検査できるようにしました。
- 明示的な Broker shutdown contract を追加し、active project sessions を revoke して保存済み bridge status を clear しつつ registered projects を保持し、shutdown counts を報告できるようにしました。
...
- `supports_live_editor_state` を欠く bridge status、compatible v3 plugin version を持たない status、または missing、malformed、古い major、新しい major、explicit version-mismatch の plugin version を報告する status からの editor-live upgrade を拒否するようにしました。
...
- 4 言語の overview release-prep link を更新し、v3.0.0 baseline が v1.4.0 note ではなく v3.0.0 release-note source を指すようにしました。
- localized docs に v3 editor bridge upgrade contract を追加し、bridge states、明示的な editor-live upgrade requirements、project/session-scoped bridge use をカバーしました。
- 4 つの localized documentation tree に v3.0.0 release-note source を初期化し、feature work の開始前にこの version line の durable release narrative file を用意しました。

### Internal

- v3 Companion project identifier を Godot project root と明示的な `.csproj` path の両方に scope し、同じ Godot root 配下の複数 C# project を Broker registration で分離できるようにしました。
- static/headless inventory project scopes を追加し、検出された各 `.csproj` と対応する scoped v3 `project_id` を公開しつつ、inventory root `project_id` を安定させました。
- static/headless default project-scope marker を追加し、単一 `.csproj` の Godot root でのみ設定することで、複数 project root では明示的な選択を必須にしました。
- static/headless project-scope selection status を追加し、複数 `.csproj` root で client が明示的な scoped `project_id` を選ぶ必要がある場合を報告できるようにしました。
- v3 project session identity に明示的な `.csproj` scope metadata を追加し、Broker client が scoped session の C# project binding を監査できるようにしました。
- v3 Companion .NET implementation を Godot addon install surface の外へ移動し、clean Asset Library install と project-discovery coverage を拡張して、plugin-owned Companion project が user .NET project として compile または discover されないようにしました。
- docs i18n validation に、各 overview page が現在の `plugin.cfg` version の release-note source に link していることを要求する coverage を追加しました。
- v3 broker manifest、validator、negative policy tests、CI coverage を追加し、broker lifecycle、transport、discovery、session-scope defaults が explicit かつ non-persistent のまま保たれるようにしました。
- machine-readable な v3 Broker tool-scope validation result と manifest coverage を追加し、missing project/session ID、unknown session、cross-project reuse、expired session、inactive session、unavailable capability を扱えるようにしました。
- v3 Broker registered-project snapshot を追加し、filesystem scan や session renewal を行わずに explicit project file scope と active static/headless、editor-live session counts を報告できるようにしました。
- v3 Broker lifecycle-options contract を追加し、companion を default で disabled のままにし、broker start と editor launch を明示的に要求するようにしました。
- v3 Broker transport-options contract を追加し、stdio を default のまま維持し、HTTP は explicit enablement、loopback-only binding、有効な port を必須にしました。
- v3 broker manifest に session lifecycle contract checks を追加し、lease、renew/stop behavior、expired-session handling、active-session limits をカバーしました。
- v3 bridge status schema、validator、negative policy tests、CI coverage を追加し、online bridge status が空でない editor session identity を必要とするようにしました。
...
- Companion contract と static analyzer の build/test gate を release workflows にも反映し、dry-run cache marker にそれらの check を追加し、companion contract workflows の active v3 baseline push filters を保護しました。
- v3 bridge contract と broker manifest validators を release workflow gates に追加し、それらの policy checks の dry-run cache markers も追加しました。
- branch policy と version-policy validator の許可 PR target に `v3.0` を追加し、v3.0 release-baseline metadata change と non-release version guardrail の coverage を追加しました。
- branch policy と version-policy validator で明示的な v3 stacked PR base branch を許可し、非 v3 target protection を緩めずに各 v3 Companion feature を独立して review できるようにしました。
...
- project と bundled plugin license を v1.4 line で MIT から Apache-2.0 に変更しました。
...
- v1.4 protocol refactor plan と progress tracker を更新し、completed MCP 2025-11-25 target、Streamable HTTP endpoint shape、newline stdio default、schema metadata policy、optional capability boundaries、public-tool cleanup、UI metadata adaptation を反映しました。
...
- protocol refactor line 用の draft v1.4 release-note sources と localized navigation を追加しました。
...
- lifecycle、JSON-RPC、Tools、Resources、Prompts、schema metadata、Streamable HTTP、stdio、Dock metadata、public-tool removals、root monolith closure、catalog facts、optional capabilities、release-facing install guidance、changelog section order、Roslyn runtime bundle shape を対象に aggregate v1.4 conformance / guardrail coverage を追加しました。
- v1.4 branch integration、trusted `refactor/v1.4.0` to `dev` version metadata handoff、version metadata changes、BOM-prefixed PR headings、formal changelog section requirements に対する release-policy、PR-policy、merge-queue validation coverage を追加しました。
