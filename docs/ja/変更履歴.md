...
- README と localized documentation entry page に Real Project Validation section を追加し、公開 Godot .NET project example への link と project screenshot を掲載しました。
...
- tool list、catalog resource、search result、Tools page rendering が `system_tool_activity` を再公開しないよう removal guard coverage を追加しました。
...
- `system_plugin_maintenance` を追加しました。status、reload、update-status、update-source selection、update-start workflow をまとめる high-level plugin maintenance entry で、専用 reload / update tool も維持します。
