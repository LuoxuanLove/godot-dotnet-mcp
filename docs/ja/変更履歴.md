...
- `system_project_run` に `action=run|stop` を追加し、project launch と stop control を 1 つの high-level entry にまとめ、`system_project_stop` は compatibility alias として維持しました。
...
- 統合された `system_project_run(action=stop)` stop path に合わせて、project-run tool tree、prompt、localization、protocol facts、harness contract を更新し、`2026-06-06.1` tool schema facts update を追加しました。
