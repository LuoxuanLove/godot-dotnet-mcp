...
- 短い redirect stub を削除し、日本語と韓国語の本文を補完して、ローカライズされた文書ツリーが placeholder や重複断片に依存しないようにしました。
- `v1.2.0` の release notes source file を pending-theme テンプレートで初期化し、プラグイン metadata の更新後に古い `v1.1.2` source file を削除しました。

### Internal

- tree shape hash、ローカライズ済み path mapping、placeholder check、同一言語 Markdown link、言語間 link 漏れを検証する docs i18n workflow を追加しました。
...
- Removed the old public Intelligence tool tree and documentation page in favor of the System tool tree and `docs/ja/モジュール/システムツール層.md`.
