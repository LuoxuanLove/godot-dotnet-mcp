...
- v2 capability manifest と validator を追加し、static/headless capability が editor-live state、default background process、listening port、または scope のない tool call を宣言できないようにしました。
...
- static/headless inventory project scopes を追加し、検出された各 `.csproj` と対応する scoped v2 `project_id` を公開しつつ、inventory root `project_id` を安定させました。
...
- v2 project session identity に明示的な `.csproj` scope metadata を追加し、Broker client が scoped session の C# project binding を監査できるようにしました。
...
- machine-readable な v2 Broker tool-scope validation result と manifest coverage を追加し、missing project/session ID、unknown session、cross-project reuse、expired session、inactive session、unavailable capability を扱えるようにしました。
- v2 Broker registered-project snapshot を追加し、filesystem scan や session renewal を行わずに explicit project file scope と active static/headless、editor-live session counts を報告できるようにしました。
- v2 Broker lifecycle-options contract を追加し、companion を default で disabled のままにし、broker start と editor launch を明示的に要求するようにしました。
- v2 Broker transport-options contract を追加し、stdio を default のまま維持し、HTTP は explicit enablement、loopback-only binding、有効な port を必須にしました。
