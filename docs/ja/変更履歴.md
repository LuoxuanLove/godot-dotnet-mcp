...
- HTTP / stdio JSON-RPC envelope handling、malformed framing、duplicate/conflicting HTTP body headers、response-envelope handling、disabled-tool parity、session validation を修正し、supported MCP transports 間の transport error を deterministic にしました。
- `/mcp` negotiation を修正しました。`initialize` は JSON-RPC body から negotiation でき、以降の requests は protocol/session headers を強制し、unsupported `Accept` は拒否され、guard responses は session を作成しません。
- Streamable HTTP SSE output を修正し、transport open / heartbeat events が undeclared logging capability に依存せず、SSE bodies が JSON response encoding で包まれないようにしました。
