...
### 準備済みの installable addon tree から

Godot Asset Library を使わない場合は、準備済みの installable addon tree だけを Godot プロジェクトへコピーします。

```text
addons/godot_dotnet_mcp
```

raw repository checkout から `addons/godot_dotnet_mcp/` を直接コピーしないでください。source tree には `dotnet_bridge/` などの internal development input が含まれます。これらは Asset Library export から除外され、user Godot project で compile されてはいけません。コピー後の addon には `plugin/runtime/roslyn_runtime/` が残っている必要があり、`dotnet_bridge/`、`companion/`、`tests/`、`scripts/` を含めてはいけません。
