...
|       | 機能 | 内容 |
| :---: | :--- | :--- |
| 🎛️ | **エディターと一緒に動く** | MCP サービスは Godot プラグインから直接提供され、追加のバックグラウンドプロセスは不要です。 |
| 🚀 | **低摩擦のセットアップ** | Godot Asset Library からインストールし、一般的な MCP クライアント向けの接続設定を生成し、GitHub ソースからプラグインを更新できます。 |
| 🎮 | **ライブの Godot エディター文脈** | 現在のシーン、選択中ノード、Dock 状態、ログ、実行情報、診断 summary、エディタースクリーンショットを Agent に提供します。 |
| 🌳 | **シーン、リソース、binding 診断** | scene tree、resource reference、dependency、scene structure issue、C# export binding state の確認を助けます。 |
| ▶️ | **ゲーム実行時サポート** | scene の起動と停止、runtime diagnostics の確認、input 実行、game runtime view の capture を行えます。 |
| 🔎 | **Roslyn ベースの C# サポート** | プラグイン内 Roslyn syntax check により、class、base type、method、enum、exported member などの C# script structure を読み取ります。 |
| 🐞 | **Godot DAP debugging** | Godot DAP 経由で breakpoint、thread、stack trace、output event を読み取り、pause、continue、step-over を実行します。managed C# breakpoint には別の .NET debugger が必要です。 |
| 📚 | **MCP Resources と Prompts** | project resource、diagnostics read entry point、一般的な Godot workflow Prompt Guide を提供します。 |
| 🧰 | **Tool extensions** | `custom_tools/` から `user_*` GDScript tool を任意で hot-load し、project 固有の MCP capability を追加できます。 |
