...
- 常规集成 PR 必须指向 `dev`；显式 v3 预开发 PR 可以指向 `v3.0`、`release/v3.0.0-baseline`，或锚定的 `feature/v3-*`、`docs/v3-*`、`ci/v3-*` stacked base branch。其它目标分支会被 `pr-policy` 拦截。
...
1. PR 指向允许的集成分支或显式 v3 stacked base，且基于当前可验证的目标 base 重新确认。
