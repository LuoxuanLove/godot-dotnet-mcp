...
- 常规集成 PR 必须指向 `dev`；显式 v2 预开发 PR 可以指向 `v2.0`、`release/v2.0.0-baseline`，或锚定的 `feature/v2-*`、`docs/v2-*`、`ci/v2-*` stacked base branch。其它目标分支会被 `pr-policy` 拦截。
...
1. PR 指向允许的集成分支或显式 v2 stacked base，且基于当前可验证的目标 base 重新确认。
