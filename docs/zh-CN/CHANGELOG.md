...
- 在 `docs/en/`、`docs/zh-CN/`、`docs/ko/` 与 `docs/ja/` 下补齐完整英文、简体中文、韩文与日文文档树，并保持各语言内部链接只指向同语言文档。
- 在 `docs/{en,zh-CN,ja,ko}/` 下新增结构化的韩文、日文与简体中文 README、变更日志和路线图入口，并从根文档补充语言切换链接。
- 使用 pending-theme 模板初始化 `v1.2.0` 发布说明源文件，并在插件元数据推进后移除过时的 `v1.1.2` 源文件。

### Internal

- 新增 docs i18n 验证 workflow，并在 guardrail 中覆盖 locale 目录一致性、文件一致性、同语言 Markdown 链接与跨语言链接泄漏检查。
...
- 更新 README、架构、运行时服务、工具系统、Tools 页、测试与 CI 文档，覆盖 MCP Resources 与 Prompts、DAP 调试器工具、`system_project_state` 紧凑读取、tool catalog resources 和 harness 验证行为。
...
- 移除旧的公开 Intelligence 工具树和对应文档页，统一改为 System 工具树与 `docs/{en,zh-CN,ja,ko}/模块/System工具层.md`。
