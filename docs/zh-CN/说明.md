...
|       | 特性 | 功能说明 |
| :---: | :--- | :--- |
| 🎛️ | **编辑器原生 MCP 服务** | 直接运行在 Godot 编辑器内，暴露实时项目、编辑器与运行时状态，无需单独后台服务。 |
| 🚀 | **安装、更新与客户端配置** | 支持从 Godot 插件商城安装、生成客户端连接配置、管理插件更新，并在 Dock 中展示客户端能力指引。 |
| 🧭 | **语义化编辑器自动化** | 为设置、Inspector 属性、菜单、弹窗、控件与 UI 等待提供高层工作流，再将鼠标式输入保留为 fallback。 |
| 🪟 | **证据化编辑器上下文** | 捕获 editor、control、popup、active-dialog 与 runtime surface，并用元数据说明目标、fallback 与观测状态。 |
| 🌳 | **项目理解与诊断** | 在编辑前检查场景、资源、依赖、输入 / 导出配置、C# 脚本结构、诊断信息与绑定一致性。 |
| ▶️ | **运行时生命周期与调试** | 启停运行时会话，等待验证 marker，检查运行时错误，发送输入，捕获帧，并读取 Godot DAP 状态。 |
| 📚 | **可发现的 MCP 工作流** | 提供本地化 resources、Prompt Guides、工具目录搜索、活动诊断与 help surface，帮助客户端选择合适工具路径。 |
| 🧰 | **可扩展 User 工具** | 热加载项目定义的 GDScript 工具，报告公开工具名与兼容性诊断，并让自定义能力对 MCP 客户端可见。 |
...
只将可安装的插件目录复制到 Godot 项目中：

```text
addons/godot_dotnet_mcp
```

不要把 `companion/`、`tests/`、`scripts/` 等仓库级开发目录复制进 Godot 项目。
...
<table>
  <tr>
    <td width="38%" valign="top"><a href="https://github.com/LuoxuanLove/CoGDSex"><img src="https://github.com/LuoxuanLove/CoGDSex/blob/main/resources/godex-workbench.png?raw=1" alt="CoGDSex Godex 工作台截图" width="420"></a></td>
    <td valign="top"><a href="https://github.com/LuoxuanLove/CoGDSex">LuoxuanLove/CoGDSex</a> 是一个公开的 Godot .NET 游戏项目，用于记录真实开发过程。它也展示了 Godot .NET MCP 在较大代码库和编辑器工作流中的实际使用，验证插件的项目能力与稳定性，并把真实改进点反馈到后续插件功能中。</td>
  </tr>
</table>
