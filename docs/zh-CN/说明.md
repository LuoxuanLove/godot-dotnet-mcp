...
如果不使用 Godot 插件商城，只将准备好的可安装插件树复制到 Godot 项目中：

```text
addons/godot_dotnet_mcp
```

不要直接从原始仓库 checkout 复制 `addons/godot_dotnet_mcp/`。源码树包含 `dotnet_bridge/` 等内部开发输入，这些内容会被 Asset Library 导出排除，也不应被用户的 Godot 项目编译。复制后的插件必须保留 `plugin/runtime/roslyn_runtime/`，且不得包含 `dotnet_bridge/`、`companion/`、`tests/` 或 `scripts/`。
