...
- 新增 `system_tool_catalog`，客户端可按关键词、分类或域搜索当前公开工具目录，并查看匹配原因、动作、参数、可见性与启用状态。
- 新增 `system_project_configure(action="list_export_presets")`，以只读摘要方式检查 `export_presets.cfg`，并遮蔽敏感 option key 与本机绝对导出路径。
- 新增 `system_project_configure(action="get_input_action")`，Agent 可通过高层项目配置工具查看单个输入动作的 deadzone 和具体事件绑定。
- 新增 `system_editor_control(action="wait_for_ui")` 有界等待与验证动作，可轮询编辑器控件的存在、可见性、文本和启用 / 禁用状态，并在成功时返回匹配的 UI 证据，失败时返回超时观测负载。
- 新增 `system_editor_control(action="get_popup")` 与 `system_editor_control(action="capture_popup")`，客户端可按返回的弹窗路径读取可见浮动编辑器弹窗 / 窗口并截取局部证据。
- 新增 `system_settings_dialog` 设置类对话框高层工作流，可打开 Project Settings 或 Editor Settings、等待可见、搜索候选设置行、聚焦结果、截图取证并关闭界面，但不写入设置值。
- 新增 `system_settings_dialog(action=list_tabs/activate_tab)` 设置标签页导航，并支持 `open(tab=...)`，可在不依赖坐标点击的情况下列出并切换 Project Settings 或 Editor Settings 标签页。
- 新增 `system_settings_dialog(action=list_categories/focus_category)`，可在设置行检查前发现设置分类树项并聚焦唯一分类，且不会写入设置值。
...
- 新增只读 `system_settings_dialog(action=focus_value)` 值编辑控件聚焦，可针对唯一可见设置行将焦点移动到值控件，且不会写入设置值。
- 新增 `system_settings_dialog(action=set_value)`，可针对受支持的唯一可见设置行通过编辑器 UI 控件写入 text、number 与 bool 值，并在操作后重新观测验证结果。
- 新增只读 `system_settings_dialog(action=verify_value)` 设置值断言，可针对唯一可见设置行比较预期 text、bool、number 与 enum 值，且不会写入设置值。
- 新增 `system_settings_dialog(action=run_task)` 可信设置任务工作流，可将打开 / 导航、唯一行解析、写入前读取、可选受支持写入、验证与截图取证编排为一个高层操作。
- 新增 `system_inspector` 高层检视器属性工作流，可针对当前编辑对象或已准备的节点 / 资源目标列出可见属性模型、解析唯一属性、读取、聚焦、设置、验证、截图取证或运行可信属性任务。
- 新增 `system_project_lifecycle(action=start|stop)` 作为规范的高层项目运行时会话生命周期入口，不再保留旧项目运行 / 停止工具名的兼容 alias。
- 新增 `system_editor_evidence` 高层视觉证据工作流，可捕获 editor、control、popup、active-dialog 或 auto surface，并显式返回目标、fallback、降级与可见弹窗元数据。
- 新增 `system_scene_inspect` 作为统一的只读场景检查入口，提供 `validate`、`analyze` 与 `full` 动作，同时保持既有场景校验与分析工具可用。
- 新增 `system_plugin_maintenance` 插件维护高层聚合入口，可统一执行状态、重载、更新状态、更新来源选择和启动更新流程，同时保留专用 reload 与 update 工具。
- 新增 User 工具命名诊断，在 User 工具列表和兼容性报告中返回脚本声明名、规范化名称与公开 MCP 工具名。
- 新增 `system_tool_activity` 查询过滤与慢调用 / 失败诊断，客户端可按状态、工具和慢调用阈值检查运行中或最近调用，而无需读取无关活动记录。
- 为 `system_editor_control` 新增点击 fallback 观测证据，使控件本地左键 / 右键点击返回已派发输入事件、Button 类目标点击前后状态、已观测控件信号，以及输入已送达但未观察到激活证据时的诊断提示。

### Changed

- 明确编辑器 UI 控制指引，让 Agent 优先使用语义工作流和导航，其次使用控件级聚焦、文本、数值与弹窗动作，仅将控件本地鼠标或指针事件作为 fallback。
...
- 扩展 MCP resources/prompts harness 覆盖，在非英文 locale 下验证 resource 与 resource template metadata 本地化。
- 扩展 Config 客户端 registry、生产检测路径与 presenter 契约覆盖，验证能力矩阵投影、兼容别名与 fallback 行为。
- 新增 system project executor 与 Tools 页渲染覆盖，验证导出预设摘要发现、敏感 option key 脱敏和绝对导出路径脱敏。
- 新增工具目录搜索 harness 覆盖，验证公开暴露、内部可见查询、按需返回 schema、过滤处理与匹配原因上报。
...
- 扩展编辑器 UI 控制原子层，覆盖 Tree 项列出与选择能力，用于设置对话框分类导航。
- 扩展 Tools 页渲染契约与文档示例，使 `plugin` domain 与 core、user 工具 domain 一起纳入覆盖。
- 修复 Tools 页标题计数，使 `plugin` domain 工具也纳入已启用和总工具数量。
- 为规范的 `system_project_lifecycle` 表面更新项目生命周期工具树、Prompt、本地化、protocol facts、stdio/router/tool-loader 与 harness 契约，并更新 `2026-06-08.16` 工具 schema facts 与显式 lifecycle action 校验。
- 扩展 Settings 页渲染覆盖，验证很长的更新引用列表既保留全部可选项，又限制选择器弹窗高度。
- 为语义优先、控件级其次、鼠标 fallback 最后的自动化顺序更新编辑器 UI Prompt、help、本地化与 schema 契约覆盖，并更新 `2026-06-08.17` 工具 schema facts。
- 扩展 settings-dialog task 契约覆盖，验证 `run_task` 读取 / 验证 / 设置 / 设置后验证流程、歧义与写入拒绝保护、要求截图时的写入前取证门控、Tools 页 action 渲染、catalog search 发现、prompt/help 指引与 `2026-06-08.19` 工具 schema facts 更新。
- 新增检视器工作流契约与工具树覆盖，验证属性模型解析、类型化值读取、受保护的 text/number/bool 写入、截图 fallback、catalog 发现、prompt/help 指引，以及 `2026-06-08.20` 工具 schema facts 更新。
- 新增编辑器证据 surface 契约、工具树条目、Tools 页渲染、catalog search 发现、prompt/help 指引、本地化工具元数据，以及 `2026-06-08.21` 工具 schema facts 更新。
- 扩展 scene executor、tool loader、Tools 页渲染、本地化 inventory 与 protocol facts 覆盖，验证 `2026-06-08.23` 的 `system_scene_inspect` schema 表面。
- 新增插件维护高层入口契约、工具树覆盖、本地化工具元数据，并将工具 schema facts 更新到 `2026-06-08.22`。
- 扩展 User 工具服务契约覆盖，验证脚手架公开名称预览、旧式 `user_` 前缀规范化 warning，以及 `2026-06-08.24` 工具 schema facts 更新。
- 扩展工具活动 registry 与 loader 契约，覆盖过滤后的 status / recent 查询、实时运行时长、慢调用摘要、有界最近失败摘要，以及 `2026-06-08.25` 工具 schema facts 更新。
- 扩展 editor/system control harness 覆盖，验证点击输入派发与 Button 类目标观测证据，同时保持语义优先的 UI 自动化指引。

### Fixed

- 加强 MCP resource diagnostics 脱敏，使大小写混合的 URL credential 与敏感文本 marker 在 resource 输出上报前被遮蔽。
- 限制 Settings 更新来源与分支选择器弹窗高度，使很长的已发现引用列表在菜单内滚动，而不会覆盖编辑器。
- 修复生成的 User 工具脚手架，使其通过资源路径继承基础工具脚本，避免 headless catalog 扫描依赖全局脚本类注册。
