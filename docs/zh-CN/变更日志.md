...
- 新增 `system_project_configure(action="list_export_presets")`，以只读摘要方式检查 `export_presets.cfg`，且不返回平台 option 值。
...
- 新增 system project executor 与 Tools 页渲染覆盖，验证导出预设摘要发现能力。
