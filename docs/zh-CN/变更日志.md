...
- 移除旧内部兼容别名 `filesystem_file`；文件操作改用 `filesystem_file_read`、`filesystem_file_write` 与 `filesystem_file_manage`。
...
- 新增移除护栏覆盖，确保 `filesystem_file` 不会重新出现在 loader definition、filesystem domain 直接执行或本地化清单中，同时保持拆分后的 filesystem 工具可用。
