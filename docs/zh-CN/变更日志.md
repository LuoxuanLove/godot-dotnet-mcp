...
- 为 `system_project_run` 新增 `action=run|stop`，将项目启动与停止控制收敛到同一个高层入口，同时保留 `system_project_stop` 作为兼容 alias。
...
- 为统一的 `system_project_run(action=stop)` 停止路径同步更新项目运行工具树、Prompt、本地化、protocol facts 与 harness 契约，并更新 `2026-06-06.1` 工具 schema facts。
