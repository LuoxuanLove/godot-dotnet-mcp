...
- 为 `system_editor_control` 与低层 `ui_control` 的 `activate_control` 新增激活观测证据，包含 Button 类目标激活前后状态、已观测信号，以及语义激活完成但无可观测证据时的提示。
...
- 扩展 editor/system control harness 覆盖，验证 `activate_control` 目标观测元数据与激活证据。
