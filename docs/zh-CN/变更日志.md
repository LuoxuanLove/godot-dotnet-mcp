...
- 将编辑器启动与空闲刷新改为轻量 MCP server startup，将 auto-start 移出 `_enter_tree` 同步路径，按需加载 tool runtime，仅为当前 Dock 页构建 catalog projection，并降低 User-tool 文件轮询频率，以减少大型项目中的编辑器卡顿。
