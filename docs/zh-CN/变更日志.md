...
- 新增 v2.0 canonical MCP resources，覆盖指南发现、项目与编辑器状态、活动状态 / 最近记录，以及公开 / 可见工具目录，同时保留既有兼容 resource URI。
...
- 新增本地化 v2.0.0 协议重构计划，并从四语路线图链接该计划。
...
- 为本地化 v2.0.0 协议计划页面新增文档本地化校验覆盖。
- 新增 v2.0.0 contract case manifest，并让插件 harness 的 required-case 选择从 manifest 标签派生。
