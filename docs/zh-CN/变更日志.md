...
- 为 README 与本地化文档入口页新增“真实项目验证”小节，链接到公开 Godot .NET 项目示例并展示项目截图。
...
- 新增移除护栏覆盖，确保工具列表、目录 resources、搜索结果和 Tools 页渲染不会重新暴露 `system_tool_activity`。
...
- 新增 `system_plugin_maintenance` 插件维护高层聚合入口，可统一执行状态、重载、更新状态、更新来源选择和启动更新流程，同时保留专用 reload 与 update 工具。
