...
- 新增 `system_plugin_maintenance` 插件维护高层入口，将状态、重载、更新状态、来源选择与启动更新流程统一到一个常用入口，同时保留 `system_plugin_reload` 与 `system_plugin_update`。
...
- 将工具 schema facts 更新到 `2026-06-05.7`，并新增 `system_plugin_maintenance_contracts` 覆盖插件维护高层入口与 Tools 树本地化。
