...
- 在 `docs/en/`、`docs/zh-CN/`、`docs/ko/` 与 `docs/ja/` 下补齐完整英文、简体中文、韩文与日文文档树，并让每个 locale 使用本语言目录名与文件名。
- 在 `docs/{en,zh-CN,ja,ko}/` 下新增结构化的韩文、日文与简体中文 README、变更日志和路线图入口，并从根文档补充语言切换链接。
- 移除短跳转 stub，并补齐日文与韩文文档正文，避免本地化树继续依赖草稿片段或重复片段。
- 在本地化文档树承载 README 与变更日志入口后，移除根目录冗余的本地化 README 与 changelog 副本。
- 修正日文与韩文文档中的本地化链接、编码示例、发布说明措辞以及 UI / Config 实现引用。
- 使用 pending-theme 模板初始化 `v1.2.0` 发布说明源文件，并在插件元数据推进后移除过时的 `v1.1.2` 源文件。

### Added

- 新增 `system_project_execution(action=run|stop)` 作为统一的高层项目生命周期入口，复用既有项目运行 marker 校验、前台窗口策略、timeout、auto-stop 与停止行为，同时保持 `system_project_run` 与 `system_project_stop` 兼容。
- 为 MCP resources 与 prompts 新增输出大小保护：超大的文件型 resource 会在完整读取前被拒绝，超长 prompt 文本会被裁剪并带上 `_meta` 字节大小详情。
- 为 `system_editor_control` 新增顶部菜单控制动作，Agent 可以列出编辑器菜单、打开 `MenuButton`，并按文本或索引选择 `PopupMenu` 菜单项，然后继续使用已有弹窗控制流程。
...
- 新增 `system_editor_control(action=select_popup_menu_item)`，Agent 可按 index、id 或精确文本选择可见 `PopupMenu` 项，并会拒绝隐藏弹窗、禁用项、分隔项、子菜单行和冲突选择器。
...
- 为 health、插件 reload 与插件 update 响应新增维护窗口契约，使客户端能在生命周期 reload 或 update sync 期间识别临时断连、重连要求、重新获取工具列表要求和重试指引。
- 为维护窗口响应输出的重连指引补齐所有支持 Dock 语言的本地化文案。
- 为 `system_editor_control` 新增 hover 与 leave 指针移动动作，便于在不使用 OS 鼠标自动化的情况下验证 tooltip、hover-only 菜单与浮动编辑器 UI。
- 新增 `system_editor_plugin_control`，客户端可检查第三方 EditorPlugin 的项目设置状态、编辑器会话状态、可见 UI 提示、重启/手动激活提示，并使用带保护的启用/禁用操作。
- 为 `system_editor_control` 新增高层编辑器控制动作，可列出当前主屏幕按钮、切换到已注册的插件主屏幕，并读取或切换专注模式。

### Fixed

- 修复 malformed JSON-RPC 参数处理：HTTP 与 stdio 请求会一致地用 `-32602` 拒绝非对象顶层 `params`，而非对象 `tools/call.arguments` 仍保持工具级 `isError` 结果。
- 修复插件更新同步流程：写入 addon 文件后会显式刷新编辑器文件系统，并在插件生命周期重载前等待刷新完成。
- 修复 C# 空方法生成逻辑：内部编辑辅助在缺少方法体时不再生成含糊的 fallback 方法体。
- 修复 HTTP transport pipelining：keep-alive 连接中已缓存的多个请求会在异步路由完成后继续 drain，不再等待新的 socket 字节才处理后续请求。
- 为多客户端恢复增加 HTTP 重连背压防护：每帧可接收多个待处理连接、拒绝过大的 pending request buffer，并在响应写入失败后关闭客户端连接。
- 为 stdio frame processing 增加异步重入、防止 stop/restart 后旧响应写回，并支持单帧限量处理 burst 请求，同时保留 tool-loader tick，提高连续 stdio 请求的稳定性。
- 修复生命周期 reload 调度失败后仍保留 pending 状态的问题。
- 修复项目文件 reimport 处理：`project.godot`、文本文件、sidecar 元数据和不支持的路径会返回结构化 `not_importable_resource` 错误，不再直接传给 Godot 导入管线。

### Internal

- 扩展 system project executor、tool loader、Tools 页、runtime-state、本地化与工具树 harness 覆盖，验证 `system_project_execution`，并将工具 schema facts 更新到 `2026-06-06.2`。
- 扩展 JSON-RPC request、HTTP server、stdio resources / prompts 与 tool router 契约，覆盖 malformed `params` 和 `tools/call.arguments` 边界，并同步 `2026-06-05.6` 工具 schema facts 更新。
- 扩展 `mcp_resources_prompts_contracts`，验证 resource 输出限制、prompt 裁剪元数据以及 `2026-06-05.5` 工具 schema facts 更新。
- 扩展 script edit service harness 覆盖，要求 C# 缺少方法体时生成显式 `NotImplementedException` guard body，而不是含糊的 fallback 方法体。
- 扩展 User 工具服务、runtime health 与 project-state harness 覆盖，验证运行时诊断和自定义工具加载失败上报。
- 扩展 plugin-evolution harness 覆盖，确保公开 runtime diagnostics 持续转发实时 User 工具 runtime snapshot。
- 提升工具 schema facts 版本，并扩展编辑器 UI harness 覆盖，验证 `list_menus`、`open_menu` 与 `select_menu_item`。
- 为工具活动 registry 扩展 router、stdio、registry 与 tool-loader harness 契约覆盖，验证 context 剥离、响应 activity 摘要、传输覆盖和公开 `system_tool_activity` 入口。
- 更新 Tools 页文档，补充 `system_tool_activity` 树节点和 `_mcp_context` 协议边界。
- 为公开 `system_tool_activity` 工具表面更新 protocol facts 中的工具 schema 版本。
- 新增 docs i18n 验证 workflow，覆盖树哈希结构一致性、本地化路径映射、草稿片段检查、同语言 Markdown 链接与跨语言链接泄漏检查。
- 扩展 docs i18n 验证，禁止应位于本地化文档树中的根目录冗余文档文件回归。
- 扩展 docs i18n 验证，捕捉 README 展示资产 locale 错配和文件职责表 path key 重复。
- 将插件元数据、协议事实文件、.NET bridge 元数据和插件更新契约 fixture 期望值切换到 `1.2.0` 开发版本线。
- 更新本地化契约覆盖，使测试验证合并后的 fallback 翻译，支持只维护本地化覆盖项的语言文件。
- 收紧本地化 CI 契约：所有支持语言必须暴露同一组 key，且除明确共享的产品名、路径、标识符和技术缩写外，不允许静默复用英文字符串。
- 修复 refactor guardrail 扫描，使 banned-source identifier 审计不再把自身验证脚本文本误报为违规。
- 扩展 HTTP transport 与 health harness 覆盖，验证稳定 connection ID、per-request audit ID、client summary 与最近断开 session 上报。
- 加固 headless HTTP server 的 runtime-control 初始化，使独立 harness server 不会把非插件 parent 传入 runtime-control service。
- 扩展 HTTP transport、health、插件 reload、插件 update 与维护契约 harness 覆盖，验证 pipelined 请求、本地化重连提示、stale-schema 状态优先级和面向重连客户端的维护窗口响应字段。
- 为 DAP 调试器增加超大 `Content-Length` frame 与 `timeout_ms` 上限保护，使异常调试端点能快速返回 `dap_limit_exceeded`，不再等待长超时。
- 扩展插件 harness 失败报告，新增 `failureClasses`、`primaryFailureClass` 与 `exitCleanupWarningFailure`，让 Godot 退出清理警告不会遮蔽运行时错误标记。
- 扩展 editor-control harness 与工具树覆盖，验证动态主屏幕发现、插件主屏幕激活和专注模式动作。
