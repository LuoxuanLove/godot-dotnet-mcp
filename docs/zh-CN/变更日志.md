...
- 修复 HTTP 与 stdio JSON-RPC envelope handling、malformed framing、重复 / 冲突 HTTP body headers、response-envelope handling、disabled-tool parity 与 session validation，使 transport error 在支持的 MCP transports 间保持确定性。
- 修复 `/mcp` negotiation：`initialize` 可从 JSON-RPC body 协商，后续 request 强制 protocol/session header，unsupported `Accept` 被拒绝，guard response 不再创建 session。
- 修复 Streamable HTTP SSE output，使 transport open/heartbeat events 不依赖未声明的 logging capability，SSE body 也不再被 JSON response encoding 包裹。
