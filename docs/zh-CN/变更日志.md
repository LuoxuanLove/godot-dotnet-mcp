...
- 新增 `system_scene_inspect` 作为统一的只读场景检查入口，提供 `validate`、`analyze` 与 `full` 动作，同时保持既有场景校验与分析工具兼容。
...
- 扩展 scene executor、tool loader、Tools 树渲染、本地化 inventory 与 protocol facts 覆盖，验证 `2026-06-06.1` 的 `system_scene_inspect` schema 表面。
