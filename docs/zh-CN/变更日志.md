...
- 为 stdio frame processing 增加异步重入防护，同时保留 tool-loader tick，提高连续 stdio 请求的稳定性。
...
- 扩展 HTTP transport、health、插件 reload 与插件 update harness 契约，覆盖 pipelined 请求和面向重连客户端的维护窗口响应字段。
