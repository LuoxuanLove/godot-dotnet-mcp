...
- `project_state(include_runtime_health=true)`：轻量健康摘要，包含 User 工具发现与加载失败诊断
...
客户端能力以稳定矩阵投影到 UI model。`ClientCapabilityMatrix` 定义每个客户端的支持层级、可用的 Config 客户端 `actions` 与说明 key；detector registry 与生产安装检测路径都会把该矩阵附加到检测结果，`ClientConfigPresenter` 再在卡片 model 中输出 `capability.support_level`、`capability.actions` 与 `capability.notes`，同时保留 `capability.kind` 作为现有引导文本使用的兼容别名。
