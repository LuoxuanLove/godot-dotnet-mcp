...
客户端能力以稳定矩阵投影到 UI model。`ClientDetectorRegistry` 定义每个客户端的支持层级、可用的 Config 客户端 `actions` 与说明 key；文件型和可执行文件型 detector 会把该矩阵透传到检测结果，`ClientConfigPresenter` 再在卡片 model 中输出 `capability.support_level`、`capability.actions` 与 `capability.notes`，同时保留 `capability.kind` 作为现有引导文本使用的兼容别名。
