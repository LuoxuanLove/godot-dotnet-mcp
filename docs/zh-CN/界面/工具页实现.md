...
- `system_*` 高层工具通过 `SystemTreeCatalog` 展开真实内部 atomic/action 链路；例如 `system_editor_control` 会展示 `click_control`、`right_click_control`、`hover_control` 与 `leave_control` 这类控件本地坐标点击和指针移动 action，以及 `list_menus` / `open_menu` / `select_menu_item` 这类顶部菜单 action，而 `system_editor_plugin_control` 会展示底层 `editor_plugin` 状态与切换 action
- `system_project_execution` 是统一的高层项目生命周期入口；它在工具树中把 `run` 与 `stop` 放在同一处，同时保留 `system_project_run` 与 `system_project_stop` 作为兼容的直接入口
