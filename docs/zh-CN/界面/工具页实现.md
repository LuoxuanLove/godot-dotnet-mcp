...
- `system_*` 高层工具通过 `SystemTreeCatalog` 展开真实内部 atomic/action 链路；例如 `system_editor_control` 会展示 `click_control` / `right_click_control` 这类控件本地坐标点击 action，以及 `list_menus` / `open_menu` / `select_menu_item` 这类顶部菜单 action
