...
- `system_*` 高层工具通过 `SystemTreeCatalog` 展开真实内部 atomic/action 链路；例如 `system_editor_control` 会展示 `click_control` / `right_click_control` 这类控件本地坐标点击 action，而 `system_editor_plugin_control` 会展示底层 `editor_plugin` 状态与切换 action
