...
- `plugin_*` 工具位于独立的 `plugin` domain 下，与 core 编辑器 / 项目工具和 user 工具分开展示
- `system_*` 高层工具通过 `SystemTreeCatalog` 展开真实内部 atomic/action 链路；例如 `system_editor_control` 会展示 `wait_for_ui`、`click_control`、`right_click_control`、`hover_control` 与 `leave_control` 这类等待验证、控件本地坐标点击和指针移动 action，以及 `list_menus` / `open_menu` / `select_menu_item` 这类顶部菜单 action，而 `system_editor_plugin_control` 会展示底层 `editor_plugin` 状态与切换 action
- `system_scene_inspect` 展示 `validate`、`analyze` 与 `full` 只读场景检查动作，同时把底层场景审计、依赖查询、绑定检查和脚本检查链路作为实现上下文保留在树中
- 活动读取迁移到 `godot-dotnet-mcp://activity/status`、`godot-dotnet-mcp://activity/recent` 与 `godot-dotnet-mcp://activity/call/{id}` 等 resources；可选 `_mcp_context` 会在具体工具执行前剥离，只作为自报协作元数据处理，不是身份认证，也不是单个工具 schema 字段
...
| `tools/tool_catalog_manifest.gd` | builtin executor 注册、domain metadata 与公开 MCP 目录事实源 |
