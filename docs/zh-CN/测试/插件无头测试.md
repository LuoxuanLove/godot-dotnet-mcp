...
| `http_server_contracts` | 验证 `mcp_http_server` 的 lifecycle、`tools/list`、`tools/call` 结构契约，以及 malformed `params` / `arguments` JSON-RPC 边界 |
...
| `json_rpc_request_service_contracts` | 验证 parse error、request object 校验、非对象 `params` 拒绝、request emission 与 router forwarding |
...
| `system_editor_control_contracts` | 验证 `impl_editor.gd` 的高层编辑器界面路由，把 `set_main_screen / activate_ui / capture_editor / list_controls / wait_for_ui / menu / capture_control / popup` 动作稳定委托到对应原子工具 |
| `system_editor_evidence_contracts` | 验证高层编辑器证据 surface 工作流，覆盖 `editor`、`control`、`popup`、`active_dialog` 与 `auto` 截图、fallback 元数据、目标必需保护和 exact capture 拒绝路径 |
| `system_settings_dialog_contracts` | 验证设置类对话框工作流，可打开 Project Settings 或 Editor Settings、等待可见、搜索候选行、列出并激活设置标签页、列出并聚焦分类树项、列出行模型、读取值、通过 `focus_value` 聚焦值编辑控件、设置并验证受支持的可见行值，覆盖 `run_task` 的读取、验证、设置、设置后验证与写入前取证门控，聚焦结果、截图取证，并通过预期原子工具关闭 |
| `system_inspector_contracts` | 验证高层检视器工作流，可列出可见属性模型、解析唯一属性、读取 text/number/bool 类型化值、聚焦值编辑器、写入并验证受支持值、拒绝不安全写入、捕获属性证据，并覆盖 `run_task` 读取 / 设置 / 验证流程 |
...
| `system_plugin_maintenance_contracts` | 验证插件维护高层入口的状态、重载、更新状态、来源选择与启动更新路由 |
...
| `tool_rpc_router_contracts` | 验证 tool list presentation、成功 tool call、缺少 tool name 错误，以及非对象 `arguments` 校验 |
