...
| `system_settings_dialog_contracts` | 验证设置类对话框工作流，可打开 Project Settings 或 Editor Settings、等待可见、搜索候选行、列出 / 聚焦分类树项、列出只读行模型、读取可见行值、聚焦结果、截图取证，并通过预期原子工具关闭 |
...
| `tools_tab_rendering_contracts` | 验证 `tools_tab.gd` 的 TreeItem 渲染、树交互、上下文菜单、Dock 内自建 Popup 坐标契约、系统 action 列表与预览回流语义 |
