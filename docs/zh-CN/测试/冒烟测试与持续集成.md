...
2. `pr-policy`: 阻止错误目标分支 PR，只允许稳定集成分支和显式 v3 stacked base，并校验标题、摘要和测试说明等客观 PR 字段
...
- PR 目标分支和轻量 PR standards 检查，错误目标分支会失败并提示改投允许的集成分支或显式 v3 stacked base，缺少必要 PR 字段时会提示补充
