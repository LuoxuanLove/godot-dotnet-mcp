...
10. `publish-release`와 `publish-plugin`은 둘 다 릴리스 전에 버전 일치, 네 언어 changelog, 네 언어 릴리스 노트 원본, 이미 존재하는 release를 검사합니다. 검사를 통과하면 영어 release note에서 GitHub Release를 만들고 로컬 패키지 자산은 올리지 않습니다.
