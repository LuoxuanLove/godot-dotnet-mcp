...
### 준비된 installable addon tree에서 설치

Godot Asset Library를 사용하지 않는 경우, 준비된 installable addon tree만 Godot 프로젝트에 복사합니다.

```text
addons/godot_dotnet_mcp
```

raw repository checkout에서 `addons/godot_dotnet_mcp/`를 직접 복사하지 마세요. source tree에는 `dotnet_bridge/` 같은 internal development input이 포함되어 있으며, 이러한 파일은 Asset Library export에서 제외되고 user Godot project에서 compile되면 안 됩니다. 복사된 addon에는 `plugin/runtime/roslyn_runtime/`가 남아 있어야 하며 `dotnet_bridge/`, `companion/`, `tests/`, `scripts/`를 포함하면 안 됩니다.
