...
- `plugin_*` 도구는 별도의 `plugin` domain 아래에 있으며, core 편집기 / 프로젝트 도구와 user 도구와 분리해 보여줍니다.
- `system_*` 상위 도구는 `SystemTreeCatalog`를 통해 실제 내부 atomic/action 경로를 펼칩니다. 예를 들면 `system_editor_control`은 `wait_for_ui`, `click_control`, `right_click_control`, `hover_control`, `leave_control` 같은 wait / 컨트롤 로컬 좌표 클릭 / pointer-motion action, `list_menus` / `open_menu` / `select_menu_item` 같은 상단 메뉴 action을 보여주고, `system_editor_plugin_control`은 하위 `editor_plugin` status/toggle action을 보여줍니다.
- `system_scene_inspect`는 `validate`, `analyze`, `full` read-only scene inspection action을 표시하며 underlying scene audit, dependency query, binding audit, script inspection chain을 implementation context로 유지합니다.
- activity 읽기는 `godot-dotnet-mcp://activity/status`, `godot-dotnet-mcp://activity/recent`, `godot-dotnet-mcp://activity/call/{id}` 같은 resources로 이동합니다. 선택적 `_mcp_context`는 실제 도구 실행 전에 제거되며, 인증이나 개별 도구 schema field가 아니라 자체 보고 coordination metadata로 처리됩니다.
