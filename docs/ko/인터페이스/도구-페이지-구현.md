...
- `system_project_execution`은 통합 high-level project lifecycle entry입니다. tree에서는 `run`과 `stop`을 같은 위치에 두며, `system_project_run`과 `system_project_stop`은 호환되는 직접 진입점으로 유지합니다.
