...
- `system_*` 상위 도구는 `SystemTreeCatalog`를 통해 실제 내부 atomic/action 경로를 펼칩니다. 예를 들면 `system_editor_control`은 `click_control` / `right_click_control` 같은 컨트롤 로컬 좌표 클릭 action과 `list_menus` / `open_menu` / `select_menu_item` 같은 상단 메뉴 action을 보여주고, `system_editor_plugin_control`은 하위 `editor_plugin` status/toggle action을 보여줍니다.
