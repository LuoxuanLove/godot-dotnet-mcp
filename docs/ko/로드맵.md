...
v1.4.0은 [protocol refactor plan](프로세스/v1.4.0-프로토콜-리팩터-계획.md)을 따릅니다. context, state, catalog는 Resources로 옮기고, workflow guidance는 Prompts에 남기며, Tools는 action과 computational workflow entry로 좁힙니다. 작업은 PR 크기의 refactor axis로 나누고 removal guards와 migration contracts를 함께 둡니다.
