...
1.x 라인은 에디터 네이티브 플러그인 구조를 계속 다듬으면서 Godot .NET MCP를 Agent 중심의 증거 우선 워크플로 플랫폼으로 만들어야 합니다. 우선순위는 Agent가 올바른 기능을 찾고, 집중된 변경을 만들고, Godot을 통해 검증하며, 신뢰할 수 있는 증거를 보고하도록 돕는 것입니다.

### v2.0.0 프로토콜 리팩터

v2.0.0은 [protocol refactor plan](프로세스/v2.0.0-프로토콜-리팩터-계획.md)을 따릅니다. context, state, catalog는 Resources로 옮기고, workflow guidance는 Prompts에 남기며, Tools는 action과 computational workflow entry로 좁힙니다. target baseline은 MCP 2025-11-25이며, `127.0.0.1:3000/mcp`를 default Streamable HTTP endpoint로, newline-delimited stdio를 default stdio mode로 삼고, schema / metadata / UI catalog adaptation을 명시적 gate로 다룹니다. 작업은 PR 크기의 refactor axis로 나누고 removal guards와 migration contracts를 함께 둡니다.

현재 실행 체크리스트는 [v2.0.0 리팩터 진행 추적](프로세스/v2.0.0-리팩터-진행-추적.md)에 기록합니다.
...
v2.0.0 프로토콜 리팩터 이후의 다음 주요 탐색 영역은 에디터 네이티브 플러그인 경계를 넘어서는 구조 확장입니다. 이는 v2.0.0 릴리스 범위가 아니며, 선택적 외부 또는 headless companion mode를 위한 향후 후보 방향입니다.

### 선택적 외부 또는 Headless Companion

companion은 에디터 네이티브 플러그인이 잘 다루기 어려운 실제 워크플로를 해결할 때만 고려해야 합니다. 가능한 목표는 다음과 같습니다.
...
Godot과 .NET debugging/tooling 경계가 실용적이라면 후속 버전은 더 깊은 .NET-oriented workflows도 탐색할 수 있습니다.
