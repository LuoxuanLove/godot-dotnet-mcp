...
v1.4.0은 [protocol refactor plan](프로세스/v1.4.0-프로토콜-리팩터-계획.md)을 따릅니다. context, state, catalog는 Resources로 옮기고, workflow guidance는 Prompts에 남기며, Tools는 action과 computational workflow entry로 좁힙니다. target baseline은 MCP 2025-11-25이며, `127.0.0.1:3000/mcp`를 default Streamable HTTP endpoint로, newline-delimited stdio를 default stdio mode로 삼고, schema / metadata / UI catalog adaptation을 명시적 gate로 다룹니다. 작업은 PR 크기의 refactor axis로 나누고 removal guards와 migration contracts를 함께 둡니다.

현재 실행 체크리스트는 [v1.4.0 리팩터 진행 추적](프로세스/v1.4.0-리팩터-진행-추적.md)에 기록합니다.
...
v3.0은 1.x 에디터 네이티브 플러그인 경계를 넘어서는 구조 확장을 탐색하기에 적절한 시점입니다. 주요 탐색 영역은 선택적 외부 또는 headless companion mode입니다.

### 선택적 외부 또는 Headless Companion

v3.0 companion은 에디터 네이티브 플러그인이 잘 다루기 어려운 실제 워크플로를 해결할 때만 고려해야 합니다. 가능한 목표는 다음과 같습니다.
...
Godot과 .NET debugging/tooling 경계가 실용적이라면 v3.0은 더 깊은 .NET-oriented workflows도 탐색할 수 있습니다.
