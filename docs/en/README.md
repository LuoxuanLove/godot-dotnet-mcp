...
### Install from a prepared installable addon tree

If you are not using the Asset Library, copy only a prepared installable addon tree into your Godot project:

```text
addons/godot_dotnet_mcp
```

Do not copy `addons/godot_dotnet_mcp/` directly from a raw repository checkout. The source tree contains internal development inputs such as `dotnet_bridge/` that are excluded from the Asset Library export and must not be compiled by a user Godot project. The copied addon must keep `plugin/runtime/roslyn_runtime/` and must not contain `dotnet_bridge/`, `companion/`, `tests/`, or `scripts/`.
