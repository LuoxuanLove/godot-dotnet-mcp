...
- Changed editor startup and idle refresh to keep MCP server startup lightweight, defer auto-start outside `_enter_tree`, lazy-load non-active Dock tabs, load tool runtimes on demand, build Dock catalog projections only for the active tab, and poll User-tool files less aggressively to reduce editor stalls in large projects.
