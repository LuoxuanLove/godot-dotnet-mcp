...
- Persisted version refs and exact commit comparisons across editor restarts. Startup now restores cache only and never requests GitHub; an explicit Refresh List refreshes stable release/tag data and the selected development branch commit data in the same request cycle, while comparison uses exact local/cache facts before one-click update requests remote verification.
