...
2. `pr-policy`: blocks PRs that target the wrong branch, allowing the stable integration branches plus explicit v3 stacked bases, and validates objective fields such as the title, summary, and testing notes
...
8. `publish-plugin`: validates pushed `v*` tags or manual release checks by confirming the tag version, `dev` reachability, release-note source files, build, harness, and rendered release-note artifact; it does not create the formal GitHub Release
...
Review:
- conversations -> resolved or pending
- Cubic latest head -> covered, pending, or issues found
```
...
- PR target branch and light PR standards checks. Wrong-target PRs fail and are told to retarget an allowed integration or explicit v3 stacked base branch, and missing required PR fields are called out
