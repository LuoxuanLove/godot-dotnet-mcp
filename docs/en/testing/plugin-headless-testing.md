...
| `system_settings_dialog_contracts` | Verify the settings-like dialog workflow that opens Project Settings or Editor Settings, waits for visibility, searches candidate rows, lists row models, reads, sets, and verifies supported visible row values, focuses results, captures evidence, and closes through the expected atomic tools |
