...
| `system_settings_dialog_contracts` | Verify the settings-like dialog workflow that opens Project Settings or Editor Settings, waits for visibility, searches candidate rows, lists/focuses category tree items, lists read-only row models, reads visible row values, focuses results, captures evidence, and closes through the expected atomic tools |
