...
This helps agents choose the right system, project, editor, runtime, script, or user-facing tool after profiles, plugin-domain tools, or user-tool state change, without relying on stale tool-name memory.

### ✅ Compatibility and Upgrade Notes

These changes only extend the public tool schema. Existing `system_editor_control` actions remain available, and clients that do not call `wait_for_ui`, `system_settings_dialog`, or `system_tool_catalog` do not need to change their requests.
