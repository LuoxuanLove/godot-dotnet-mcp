...
- Added `system_tool_catalog`, a searchable tool directory that helps clients find available tools by name, description, action, parameter, category, or domain.
...
- `system_tool_catalog` is additive and read-only; clients that do not call it need no migration.
