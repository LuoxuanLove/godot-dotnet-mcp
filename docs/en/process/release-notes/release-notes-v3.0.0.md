...
Godot .NET MCP v3.0.0 starts the optional Companion direction with a project-scoped .NET contract layer. The first pieces define how static project analysis stays available without an open editor, and how editor-live automation becomes available only after a matching Godot editor bridge is online.
...
Tool calls must carry both `project_id` and `session_id`, and the broker rejects attempts to reuse a session across a different project. This gives the v3.0 line a clear isolation boundary before multi-project orchestration is added.
...
The broker also prunes expired sessions before opening new ones and keeps active session counts bounded per project and across the broker. This makes the early v3 session model honest about lifetime and capacity before it becomes a long-running companion process.
