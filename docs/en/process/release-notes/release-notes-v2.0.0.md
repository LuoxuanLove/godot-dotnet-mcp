...
The Settings page keeps port, log level, language, Release/Development source controls, branch selection, Refresh List, one-click update, and row-level Switch actions, but presents them in a quieter hierarchy. Redundant policy and repository text is removed, the Development branch selector moves above the update actions, and stable and development version histories remain visible together in separate panels. Necessary diagnostics stay in the wrapping primary status, while long metadata is ellipsized with its complete value available as a tooltip.

Each stable and development panel uses three focused columns, marks the current row with text as well as color, and moves dates to row tooltips. Selecting a commit highlights the whole row and makes that exact ref/commit the comparison target; only the separate Switch action changes the installed version. Selection and startup are cache-only. Refresh List fetches stable release/tag metadata, the selected development branch's commit metadata, and bounded, paginated commit histories for the installed source and selected target in one request cycle, then calculates ahead/behind counts locally. One-click update uses that refreshed local verification and does not call GitHub's Compare endpoint. The explicit Switch action downloads the selected archive and, for a selected branch, resolves its ref when cached commit metadata is unavailable. Forms, actions, and the two version panels stack into a true single-column layout at ultra-narrow Dock widths, so the update workflow remains usable without horizontal clutter.
...
- Settings restores cached update refs and complete commit histories at startup without contacting GitHub. Refresh List updates both visible channel panels and the installed/selected histories, and one-click update uses the resulting local comparison. If that history cannot establish a safe relationship, use the explicit Switch action instead.
