...
The version list now uses three focused columns, marks the current row with text as well as color, and moves dates to row tooltips. Selecting a commit highlights the whole row and makes that exact ref/commit the comparison target; only the separate Switch action changes the installed version. Selection and startup are cache-only. Refresh List fetches the selected channel's metadata, one-click update may request remote comparison, and the explicit Switch action downloads the selected archive and resolves its ref when cached commit metadata is unavailable. Forms and actions stack into a true single-column layout at ultra-narrow Dock widths, so the update workflow remains usable without horizontal clutter.
