...
- The language menu is shown as `self-name (localized name)`, for example `English (英文)` or `日本語 (日语)`
...
1. `plugin.gd._build_dock_model()` gathers state, the tool list, config text, and `editor_scale`
...
- If you need main-project runtime debug information, prefer `debug.runtime_bridge`. It returns runtime-bridge events and editor debugger session state instead of a full mirror of the native Output or Debugger panels
