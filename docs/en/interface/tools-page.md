...
- `system_*` high-level tools expand through `SystemTreeCatalog` into their real atomic and action chains. For example, `system_editor_control` shows control-local click actions such as `click_control` / `right_click_control` and top menu actions such as `list_menus` / `open_menu` / `select_menu_item`, while `system_editor_plugin_control` shows the underlying `editor_plugin` status and toggle actions
