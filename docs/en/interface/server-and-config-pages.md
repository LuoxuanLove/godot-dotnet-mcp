...
Client capability is projected as a stable matrix. `ClientCapabilityMatrix` defines each client support level, the available Config-client `actions`, and note keys. Both the detector registry and the production install-detection path attach that matrix to detection results, and `ClientConfigPresenter` exposes `capability.support_level`, `capability.actions`, and `capability.notes` in the card model while keeping `capability.kind` as the compatibility alias used by the existing guidance text.
