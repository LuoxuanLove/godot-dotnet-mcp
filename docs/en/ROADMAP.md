...
The 1.x line should keep improving the editor-native plugin architecture while turning Godot .NET MCP into an evidence-first workflow platform for Agents. The first goal is to help the Agent discover the right capabilities, make focused changes, verify them in Godot, and report trustworthy evidence.
...
v2.0 is the right stage to explore architecture extensions beyond the 1.x editor-native plugin boundary. The main area to explore is an optional external or headless companion mode.
